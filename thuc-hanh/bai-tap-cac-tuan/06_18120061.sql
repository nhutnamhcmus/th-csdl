...
-- Q58. Cho biết tên giáo viên nào mà tham gia đề tài đủ tất cả đề tài
-- Kết quả: Tên giáo viên
-- Chia: chủ đề
-- Bị chia: tham gia đề tài, đề tài
SELECT gv.HOTEN
FROM dbo.GIAOVIEN as gv
WHERE NOT EXISTS (SELECT cd.MACD
					FROM dbo.CHUDE AS cd
					EXCEPT
					SELECT dt.MACD
					FROM dbo.THAMGIADT as tg JOIN dbo.DETAI as dt ON tg.MADT = dt.MADT
					WHERE gv.MAGV = tg.MAGV
					)
-- Q59. Cho biết tên đề tài nào mà được tất cả các giáo viên của bộ môn HTTT tham gia
-- Kết quả: Tên đề tài
-- Chia: giáo viên bộ môn HTTT
-- Bị chia: Tham gia, đề tài
SELECT dt.TENDT
...
FROM dbo.DETAI AS dt
WHERE NOT EXISTS( SELECT gv.MAGV
					FROM dbo.GIAOVIEN AS gv JOIN dbo.BOMON AS bm ON gv.MABM = bm.MABM
...
WHERE NOT EXISTS (SELECT TG.MADT
					FROM dbo.THAMGIADT AS TG JOIN dbo.GIAOVIEN AS GV ON TG.MAGV = GV.MAGV
...
FROM dbo.GIAOVIEN AS gv
WHERE NOT EXISTS (SELECT DT.MADT
					FROM dbo.DETAI AS DT JOIN dbo.CHUDE AS CD ON DT.MACD = CD.MACD
...
FROM dbo.GIAOVIEN AS gv
WHERE NOT EXISTS (SELECT DT.MADT
					FROM dbo.DETAI AS DT JOIN dbo.GIAOVIEN AS GVCN ON DT.GVCNDT = GVCN.MAGV
...
FROM dbo.GIAOVIEN AS gv
WHERE NOT EXISTS (SELECT CV.SOTT
					FROM dbo.CONGVIEC AS CV JOIN dbo.DETAI AS DT ON CV.MADT = DT.MADT
					WHERE DT.TENDT = N'Nghiên cứu tế bào gốc'
					EXCEPT
					SELECT tg.STT
					FROM dbo.THAMGIADT AS tg JOIN dbo.DETAI AS dt ON tg.MADT = dt.MADT
					WHERE tg.MAGV = gv.MAGV AND dt.TENDT = N'Nghiên cứu tế bào gốc'
					)

-- Q69. Tìm tên các giao viên được phân công làm tất cả các đề tài có kinh phí trên 100 triệu
SELECT gv.HOTEN
FROM dbo.GIAOVIEN AS gv
WHERE NOT EXISTS (SELECT dt.MADT
					FROM dbo.DETAI AS dt 
...
FROM dbo.GIAOVIEN AS gv JOIN dbo.GIAOVIEN AS GVQL ON gv.MAGV = GVQL.MAGV
JOIN dbo.BOMON AS bm ON gv.MABM = bm.MABM
WHERE NOT EXISTS (SELECT DT.MADT
					FROM dbo.DETAI AS DT JOIN dbo.CHUDE AS CD ON DT.MACD = CD.MACD
